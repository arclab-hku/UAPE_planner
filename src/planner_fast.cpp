...
{
return !((ct_center.head(3) - camera_vertex.col(0)).dot((camera_vertex.col(0) - camera_vertex.col(1)).cross(camera_vertex.col(0) - camera_vertex.col(2)))<0
  ||(ct_center.head(3) - camera_vertex.col(0)).dot((camera_vertex.col(0) - camera_vertex.col(2)).cross(camera_vertex.col(0) - camera_vertex.col(3)))<0
  ||(ct_center.head(3) - camera_vertex.col(0)).dot((camera_vertex.col(0) - camera_vertex.col(3)).cross(camera_vertex.col(0) - camera_vertex.col(4)))<0
  ||(ct_center.head(3) - camera_vertex.col(0)).dot((camera_vertex.col(0) - camera_vertex.col(4)).cross(camera_vertex.col(0) - camera_vertex.col(1)))<0
  ||(ct_center.head(3) - camera_vertex.col(1)).dot((camera_vertex.col(1) - camera_vertex.col(4)).cross(camera_vertex.col(1) - camera_vertex.col(2)))<0);
}
...
//   }
 if ((last_check_pos - start).norm() >0.5 || (ros::Time::now()-last_check_time).toSec()>0.5)
{gen_polyhedrons(obs_pointer);
 last_check_pos = start;
 last_check_time =ros::Time::now(); }
// if_config = true;
total_t = traj.getTotalDuration();
if ((traj.getPos(total_t)-wPs.back()).norm() > 0.5) return false;

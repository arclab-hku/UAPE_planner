...
if __name__ == '__main__':
    ...
    print("begin!")
    convert=convert_pcl()
    convert.fx, convert.fy, convert.cx, convert.cy =  [305.816284, 305.47708, 211.019928, 120.3489976] 
    convert.objects = []
   # c_track = track()
    convert.listener()
    # convert.vel=None
...
#            pcl1=np.array(pcl1)
            ...
            if len(pcl_c2) and len(pcl_c1):# and len(index_c1)==len(pcl_c1) and len(index_c2)==len(pcl_c2) and index_c1.any() and index_c2.any():
               ...
               # t_dbscan_1 = time.time()
                ...
                matched = []
                if   (convert.objects !=[]):
                    for obi in range(len(convert.objects.bounding_boxes)):
                        if convert.objects.bounding_boxes[obi].Class != "person":
                            continue
                        zc = convert.objects.bounding_boxes[obi].distance
                        xc = ((convert.objects.bounding_boxes[obi].xmin+convert.objects.bounding_boxes[obi].xmax)/2 - convert.cx) * zc / convert.fx
                        yc = ((convert.objects.bounding_boxes[obi].ymin+convert.objects.bounding_boxes[obi].ymax)/2 - convert.cy) *zc / convert.fy

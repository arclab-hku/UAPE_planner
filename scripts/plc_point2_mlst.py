...
#        self.octo_plc = list(point_cloud2.read_points(data, field_names=("x", "y", "z"), skip_nans=True))
        if len(data.points):
            ...
            self.octo_timestamp =data.header.stamp
            for o_p in data.points:
                self.octo_plc.append([o_p.x,o_p.y,o_p.z])
        # print("self.octo_plc",self.octo_plc)
#        print(self.octo_plc)
    def path_callback(self,path):
        ...
        print("PL call:",PL)
        if ros_t - self.path_time > 1 and len(PL)>1:  # path timeout
...
#                                      self.callback,queue_size=1,buff_size=52428800) #/filtered_RadiusOutlierRemoval',
        self.octo_plc_sub = rospy.Subscriber('/local_map',
                                          Marker,
...
#                            plc1.append(point)
                if len(pos_h)!=0 and np.linalg.norm(np.array([px,py,pz])-pos_h[-1])<0.1 and len(plc)>n_p :
                    plc_c[:,0]=plc[:,2]+0.07  #distance from camera to uav center
                    plc_c[:,1]=-plc[:,0]
                    plc_c[:,2]=-plc[:,1]
...
#                                        plc_sta=plc_sta+clu_p2[i].tolist()
            ...
            if len(plc1)>0 and local_pos is not None:
                ...
                #     else:
                #         plc1=plc1[::2]
                plc1=convert.distance_filter(plc1,3)
                plc1=plc1+local_pos
...
#            print(octo_plc)
            ...
            (trans,rot) = convert.tflistener.lookupTransform('map', 'local_frame', rospy.Time(0))
            trans=np.array(trans)
            # print("trans,rot",trans,rot)
            if len(octo_plc)>0:
                # octo_plc=octo_plc+trans
                convert.dis_2th_wp = max(3,convert.dis_2th_wp)
                octo_plc=octo_plc[(abs(octo_plc)<convert.dis_2th_wp).all(axis=1)]
                octo_plc=octo_plc[(octo_plc+trans)[:,2]>0.1]
                octo_plc=convert.distance_filter(octo_plc+trans-local_pos,convert.dis_2th_wp)+local_pos
                octo_plc=np.r_[octo_plc,np.array([convert.wp_1th,convert.wp_2th,convert.wp_eth])]
                # print("octo_plc",octo_plc)
                point22=convert.xyz_array_to_pointcloud2(octo_plc,'map',convert.octo_timestamp)

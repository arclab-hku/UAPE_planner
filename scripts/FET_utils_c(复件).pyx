...
@cython.cdivision(True) 
@cython.profile(True) 
cdef list check_fov2(np.ndarray[FTYPE_t, ndim=2] samples,list ids,list cam_param,list cam_rec,np.ndarray[FTYPE_t, ndim=1] uav_pose):
...
@cython.cdivision(True) 
@cython.profile(True)       
cpdef opt_path (np.ndarray[FTYPE_t, ndim=2] wpts,np.ndarray[FTYPE_t, ndim=2] pcl,double SA):
    cdef int i
    if len(wpts)>3:
        i=1
        while i < len(wpts)-1:
...
@cython.cdivision(True) 
@cython.profile(True)  
cdef get_dyn_node (np.ndarray[FTYPE_t, ndim=3] dyn_obs, np.ndarray[FTYPE_t, ndim=2] V_A, np.ndarray[FTYPE_t, ndim=1] last_pos, np.ndarray[FTYPE_t, ndim=1] it,double last_t)
    cdef np.ndarray[FTYPE_t, ndim=1] c1,c2,c3
    double am = 6
    cdef np.ndarray[FTYPE_t, ndim=1] c1 = V_A[0]
    cdef np.ndarray[FTYPE_t, ndim=1] c2 = V_A[1]/2
    cdef list t,tt
    for i in range(3):
        cdef np.ndarray[FTYPE_t, ndim=1] b_Ac1 = c1[i]**2 + (2/3*c2[i]+am/6)*(it[i]-last_pos)
        cdef np.ndarray[FTYPE_t, ndim=1] b_Ac2 = c1[i]**2 + (2/3*c2[i]-am/6)*(it[i]-last_pos)
        if b_Ac1 >0:
        ...
        if b_Ac2 >0:
            ...
            t.append((-c1[i] - (b_Ac2)**0.5)/(2*(2/3*c2[i]-am/6)))
        t = np.array()
        tt.append(min(np.array(t)[np.where(np.array(t)>0)[0]]))
    cdef double T = max(tt)
    cdef np.ndarray[FTYPE_t, ndim=1] c3 = (it[i]-last_pos - c1*T - c2*T**2)/T**3
    np.ndarray[FTYPE_t, ndim=2] ct_center
    double ct
    for i in range(int(T/0.1)+1):
        ct = 0.1*i
        ct_center = (ct+last_t)* dyn_obs[:,-1,:] + dyn_obs[:,0,:]
        if ((abs((last_pos +  c1*ct + c2*ct**2 +  c3*ct**3) - ct_center) - dyn_obs[:,1,:]) < 0).any():
            return [],True
        if i == int(T/0.1):
...
cpdef get_path (np.ndarray[FTYPE_t, ndim=2] pcl,np.ndarray[FTYPE_t, ndim=1] goal,double dr,double sense_range,double SA,
    ...
    cdef int if_cld0,check_num,j,circle_num,jj,if_cld,index,iter_num,check_fov_num,which_len,ii
    cdef np.ndarray[FTYPE_t, ndim=2] pts,samples,last_nodes,cost_list,new_costs,final_waypoints
    cdef np.ndarray[FTYPE_t, ndim=3] wpts,new_wpts
    ...
    for j in which_collide:
        if iter_num==0:
            ...
            wpts_dyn = np.array([[np.array([[0,0,0],V_A[0],V_A[1],[0,0,0]]).astype(float)]*path_num])
        ...
        while len(ct_nodes) < path_num and circle_num<len(seeds[j]):
            ...
            idx_fov,failed= check_fov2(samples,ids,cam_param,cam_rec,uav_pose)
            ...
            print("samples used for collision check:", samples[idx_fov]+uav_pose[0:3],uav_pose[0:3],goal)
            for it in samples[idx_fov]:
                ...
                t12 = time()
                index,node = get_node(wpts,it,dis_layer)
                if index<0:
                    continue
                ...
                cdef int if_dyn_cld =1
                cdef np.ndarray[FTYPE_t, ndim=2] dyn_node
                if not if_cld : 
                    dyn_node, if_dyn_cld = get_dyn_node (dyn_obs,wtps[-1,index,1:3,:],node,it,wtps[-1,index,-1,0])  #V_A should be the state of the last node
